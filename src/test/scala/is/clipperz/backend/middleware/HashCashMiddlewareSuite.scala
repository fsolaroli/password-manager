...
object HashCashMiddlewareSpec extends ZIOSpecDefault:
    ...
    val idApp: Routes[TollManager & SessionManager & Tracing, Nothing] = Routes(Method.ANY / "api" / "blobs" / string("c") -> Handler.ok) @@ hashcash(ChallengeType.MESSAGE, ChallengeType.MESSAGE)
    ...
    ).provideSomeLayer(layers) @@ TestAspect.sequential
